...
[Bharatviz](https://bharatviz.saketlab.org) provides a REST API for generating publication-ready choropleth maps of India.
...
```{r bharatviz_api, fig.width=10, fig.height=6}
library(httr)

bharatviz_map <- function(data, title = "", color_scale = "spectral", legend_title = "°C") {
  data_list <- lapply(seq_len(nrow(data)), function(i) {
    list(state = data$state[i], value = data$value[i])
  })

  response <- POST(
    url = "https://bharatviz.saketlab.org/api/v1/states/map",
    body = list(
      data = data_list,
  ...
  if (status_code(response) == 200) {
    ...
    if (isTRUE(result$success) && length(result$exports) > 0) {
      base64_str <- result$exports[[1]]$data
      img_data <- base64enc::base64decode(base64_str)
      img <- png::readPNG(img_data)
      return(grid::rasterGrob(img, interpolate = TRUE))
    }
  }
  # Return empty grob on failure
  grid::nullGrob()
}

era5_grob <- bharatviz_map(era5_api_data, title = "ERA5 Mean Temp (2023)")
imd_grob <- bharatviz_map(imd_api_data, title = "IMD Mean Temp (2023)")

grid::grid.newpage()
grid::pushViewport(grid::viewport(layout = grid::grid.layout(1, 2)))
grid::pushViewport(grid::viewport(layout.pos.col = 1))
grid::grid.draw(era5_grob)
grid::popViewport()
grid::pushViewport(grid::viewport(layout.pos.col = 2))
grid::grid.draw(imd_grob)
grid::popViewport()
```
